...
CALL_TIMEOUT_SECONDS = 20  # How long to wait for a call to be answered

# === Threading control events for graceful shutdown ===
...
def trigger_alert_and_update_db(db_id, description):
    ...
    alert_message = f"⚠️ ALERT: {description} detected at VIT Pune. Please respond immediately."
...
def finalize_clip(out, current_clip_basename):
    """Finalize video only (snapshot now happens at detection)"""
    if out:
        out.release()
    os.makedirs(output_dir, exist_ok=True)
...
def run_live_camera(chunk_size=30, clip_duration=10, camera_index=0):
    ...
    fps, width, height = 30, int(cap.get(3)), int(cap.get(4))  # default FPS for webcam
